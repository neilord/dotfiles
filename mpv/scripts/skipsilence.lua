...
local function get_saved_time(now)
    ...
    if saved > -0.001 and saved <= 0 then
        saved = 0
    end
...
local function format_info(style, saved_total, period_current, saved)
    ...
    local silence_stats = ("Saved total: %.3fs\nLatest: %.3fs, %.3fs saved"):format(saved_total, period_current, saved)
    ...
    return "Status: "
        ...
        .. "Arnndn: "
        ..
        (opts.arnndn_enable and opts.arnndn_modelpath ~= "" and "enabled" .. (opts.arnndn_output and " with output" or "") or "disabled")
        .. "\n"
...
local function update_info(now)
    ...
    if opts.infostyle == "total" or opts.infostyle == "compact" or opts.infostyle == "verbose" then
        local s = speed_stats
        if
            (opts.infostyle == "total" or opts.infostyle == "compact")
            and s.saved_total + s.saved_current == 0
            and s.time == nil
        then
            return false
        end
...
local function set_option(opt_name, value)
    mp.commandv("change-list", "script-opts", "append", mp.get_script_name() .. "-" .. opt_name .. "=" .. value)
end
...
local function cycle_info_style(style)
    local value = style
        or (
            opts.infostyle == "total" and "compact"
            or (opts.infostyle == "compact" and "verbose" or (opts.infostyle == "verbose" and "off" or "total"))
        )
...
local function info(style)
    mp.osd_message(format_info(style or opts.infostyle, get_current_stats(mp.get_time())))
end
...
(require("mp.options")).read_options(opts, nil, function(list)
    ...
    if
        list["threshold_db"]
        or list["threshold_duration"]
        or list["arnndn_enable"]
        or list["arnndn_modelpath"]
        or list["arnndn_output"]
    then
        if is_enabled then
            reapply_filter()
        end
    ...
    if
        list["ramp_constant"]
        or list["ramp_factor"]
        or list["ramp_exponent"]
        or list["speed_updateinterval"]
        or list["speed_max"]
    then
        if is_enabled then
            check_time()
...
mp.add_key_binding("F1", "toggle", toggle)
mp.register_script_message("adjust-threshold-db", adjust_thresholdDB)
mp.add_key_binding("F2", "threshold-down", function()
    adjust_thresholdDB(-1)
end, "repeatable")
mp.add_key_binding("F3", "threshold-up", function()
    adjust_thresholdDB(1)
end, "repeatable")
mp.add_key_binding(nil, "info", info, "repeatable")
mp.add_key_binding(nil, "cycle-info-style", cycle_info_style, "repeatable")
mp.add_key_binding(nil, "toggle-arnndn", function()
    toggle_option("arnndn_enable")
end)
mp.add_key_binding(nil, "toggle-arnndn-output", function()
    toggle_option("arnndn_output")
end)
