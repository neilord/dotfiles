...
local o = {
	...
	[ ["B", "all"], ["k", "keybinds"], ["K", "keybinds"], ["r", "recents"], ["R", "recents"], ["d", "distinct"], ["D", "distinct"], ["f", "fileonly"], ["F", "fileonly"] ]
	...
	header_text =
	"🔖 Bookmarks [%cursor%/%total%]%prehighlight%%highlight%%afterhighlight%%prefilter%%filter%%afterfilter%%presort%%sort%%aftersort%%presearch%%search%%aftersearch%", --Text to be shown as header for the list
		...
		for i = 1, #list_contents do
			if o.search_behavior == "specific" then
				...
				elseif string.lower(list_contents[i].found_datetime):match(string.lower(search_query)) then
					table.insert(filtered_table, list_contents[i])
				elseif
					list_contents[i].found_slot
					and string
					.lower(get_slot_keybind(tonumber(list_contents[i].found_slot)))
					:match(string.lower(esc_string(search_string)))
				then
					table.insert(filtered_table, list_contents[i])
				end
...
function draw_list()
	...
	for i = list_start, list_start + o.list_show_amount - 1, 1 do
		...
		if list_contents[#list_contents - i].found_slot then
			osd_msg = osd_msg
				.. o.keybinds_seperator
				.. get_slot_keybind(tonumber(list_contents[#list_contents - i].found_slot))
		end
...
function add_slot_log_entry()
	...
	for i = #list_contents, 1, -1 do
		if list_contents[i].found_path == filePath and tonumber(list_contents[i].found_time) == seekTime then
			list_contents[i].found_line = list_contents[i].found_line:match(
				"(.* | " .. esc_string(log_time_text) .. "%d*%.?%d*)(.*)$"
			) .. " | " .. log_keybind_text .. slotKeyIndex
			break
		end
	end
	...
	for i = 1, #list_contents do
		for j = 1, #list_highlight_cursor do
			if list_contents[#list_contents + 1 - i] then
				if
					list_contents[#list_contents + 1 - i].found_sequence == list_highlight_cursor[j][2].found_sequence
				then
		...
		if list_contents ~= nil and list_contents[1] then
			...
			if slot_taken then
				...
				if o.keybinds_empty_auto_create then
					...
					if o.osd_messages == true then
						mp.osd_message(
							"No Bookmark Slot For" .. o.keybinds_seperator .. get_slot_keybind(slotKeyIndex) .. " Yet"
						)
					end
					msg.info(
